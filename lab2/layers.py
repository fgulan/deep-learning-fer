...
def softmax(x):
    exp_x = np.exp(x)
    exp_sums = np.sum(exp_x, axis=1, keepdims=True)
...
class FC(Layer):
  ...
  def backward_params(self, grads):
    ...
    """
    grad_weights = np.dot(grads.T, self._inputs)
    grad_bias = np.sum(grads, axis=0)
    return [(self.weights, grad_weights), (self.bias, grad_bias)]
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    x_softmax = softmax(x)
    return -np.sum(np.log(x_softmax) * y) / x.shape[0]
...
class L2Regularizer():
  ...
  def backward_params(self):
    ...
    return [(self.weights, grad_weights)]
